...
func NewAPIClient(baseURL string, method string, username string, password string, ) *APIClient {
	return &APIClient{
		BaseURL:  baseURL,
		Client:   &http.Client{},
		method:   method,
...
func (api *APIClient) DoRequest(payload interface{}) (*http.Response, error) {
	url := api.BaseURL
	...
	if payload != nil {
		...
		req, err = http.NewRequest(api.method, url, bytes.NewBuffer(jsonData))
		req.Header.Set("Content-Type", "application/json")
	} else {
		req, err = http.NewRequest(api.method, url, nil)
	}
	if err != nil {
		return nil, fmt.Errorf("error creating %v request: %v", api.method, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("error performing %v request: %v", api.method, err)
	}
